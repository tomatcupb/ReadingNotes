...
操作重点：commit操作，分支管理，远程协作

概念重点：几个重要的对象（commit、blob等），内部结构的设计（HEAD，分支标签等）
...
```
git checkout cat+git push//在设置upstream之后,切换到cat分支，直接推送到远端，创建或更新cat远程分支
git push origin cat		//把本地的cat分支推送到远程仓库，并创建或更新名为cat的远程分支
git push origin cat:cat	//上一条指令的完整版
git push origin cat:dog	//把本地的cat分支推送到远程仓库的dog分支（创建或更新名为dog的远程分支）
```
...
1. 汤姆fork了一个项目，杰瑞也fork了这个项目。
2. 杰瑞在本地完成项目修改后push到自己的远程仓库，并发起PR。
3. 杰瑞的PR被通过，合到了原项目中。
4. 汤姆在本地完成项目修改后push到自己的远程仓库，并发起PR。
5. 由于原项目版本已经领先，发起PR失败。

```
1. 添加原项目仓库为远端节点
查看项目当前的远端节点
$ git remote -v
origin  https://github.com/tomatcupb/test_remote.git (fetch)
origin  https://github.com/tomatcupb/test_remote.git (push)
添加原项目的远程仓库为远端节点
$ git remote add new_origin https://github.com/tomatcupb/test_remote2.git

2. 抓取原项目的内容
git fetch new_origin
git merge new_origin/master
...
删除远端的cat分支。结合2.12.1，可以认为是推一个空的内覆盖到远端的cat分支上，即为删除cat分支。
git push origin :cat
```

#### 2.5 进阶操作

##### 2.5.1 修改历史记录（高阶rebase用法）

**合并commit记录**

```
git rebase -i bb0c9c2（最初的版本号）
配合vim修改提交记录（pick->squash,删除之前的提交信息并修改最近的提交信息）
因为rebase修改了历史记录，必须利用git push -f强制push到远程仓库，否则push出错
```

![](E:\GitWorkspace\ReadingNotes\git_rookie_to_master\images\git_rebase1.jpg)

![](E:\GitWorkspace\ReadingNotes\git_rookie_to_master\images\git_rebase2.jpg)
